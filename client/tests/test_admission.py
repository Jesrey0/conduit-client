...
class SourceAdmissionTests(unittest.TestCase):
    def envelope(self, root: Path):
        ...
        return {"schemaVersion":4,"provisioningId":"prov_test","purpose":"ENROLLMENT","lifecycle":"probation","authorization":{"state":"AUTHORIZED_TO_REQUEST_ENROLLMENT","accessClass":"LIVE_PROBATION","issuedAt":datetime.now(timezone.utc).isoformat(),"expiresAt":(datetime.now(timezone.utc)+timedelta(hours=1)).isoformat(),"approvalRequired":True},"server":{"url":"https://example.test/conduit-local"},"client":{"repository":"https://github.com/Jesrey0/conduit-client","commit":commit,"entrypoint":"client/conduit_admission.py","minimumPython":"3.12"},"requestedGrant":{"privileges":["inspect"],"workspaceIds":["playground"],"defaultWorkspaceId":"playground"},"invite":{"id":"inv_test","secret":"private","singleUse":True},"credentialHandling":{"resultingAuthPath":"/home/user/.conduit_auth.json","enrollmentResumePath":"/home/user/.conduit_enrollment.json","admissionReportPath":"/home/user/.conduit_admission_report.json","requiredMode":"0600"},"signature":{"algorithm":"Ed25519","keyId":"test-key","value":"AA"}}
    def test_loads_v3_and_verifies_clone_pin(self):
        root=Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)/"p.json"; p.write_text(json.dumps(self.envelope(root)))
            with patch.object(admission,"verify_envelope_signature",return_value="test-key"):
                d=admission.load_envelope(p)
            source=admission.verify_source(d)
            self.assertEqual(source["commit"],d["client"]["commit"])
    def test_signature_failure_precedes_network_contact(self):
        root=Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)/"provisioning.json"; p.write_text(json.dumps(self.envelope(root)))
            with patch.object(admission,"verify_envelope_signature",side_effect=ValueError("bad signature")), patch.object(admission,"post") as post:
